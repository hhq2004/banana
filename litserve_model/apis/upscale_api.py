...
from modules.icon_picture_processor import UpscaleModel
...
class UpscaleLitAPI(ls.LitAPI):
    def setup(self, device):
        ...
        self.device = device
        _log("upscale", f"setup device={device}")
        self.model = UpscaleModel(model_path=upscale_config.get('model_path'),
                                  device=device)
        self.model.load()
    ...
    def predict(self, x, **kwargs):        
        ...
        _log("upscale", f"predict batch={len(x)} device={getattr(self, 'device', 'unknown')}")
        results = []
        for idx, item in enumerate(x):  # x is a list of requests
            image = item["image"]

            start = time.time()
            result = self.model.upscale(image=image)
            elapsed = round(time.time() - start, 3)

            _log("upscale", f"worker_pid={os.getpid()} idx={idx} elapsed={elapsed}s result_type={type(result).__name__}")
