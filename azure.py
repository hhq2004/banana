...
class TextBlock:
    ...
    has_size_variation: bool = False  # 是否包含字体大小变化（可能有下标/上标）
    word_sizes: List[float] = field(default_factory=list)  # 每个单词的字体大小
...
class OCRResult:
    ...
    word_polygons: List[List[Tuple[float, float]]] = field(default_factory=list)
...
class AzureOCR:
    ...
    def _parse_result(self, result: dict, width: int, height: int) -> OCRResult:
        """解析 API 返回结果"""
        text_blocks = []
        word_polygons: List[List[Tuple[float, float]]] = []
        
        # 解析文字行
        for page in result.get('pages', []):
            for word in page.get('words', []):
                polygon = self._extract_polygon(word.get('polygon', []))
                if polygon:
                    word_polygons.append(polygon)
            for line in page.get('lines', []):
                polygon = self._extract_polygon(line.get('polygon', []))
                font_size_px = self._estimate_font_size(polygon)
                
                # 通过 polygon 形状检测是否可能包含下标/上标
                # （不依赖 words 字段，因为 Azure OCR 不一定返回）
                has_size_variation = self._detect_subscript_from_polygon(polygon, line.get('content', ''))
    ...
    def _detect_size_variation(self, word_sizes: List[float], threshold: float = 0.7) -> bool:
        """
        检测单词列表中是否存在字体大小变化（可能表示有下标/上标）
        
        Args:
            word_sizes: 单词字体大小列表
            threshold: 大小差异阈值（默认0.7，即小于主体字号70%认为是下标）
        ...
        # 找出最大字号（主体字号）
        max_size = max(word_sizes)
        if max_size <= 0:
            return False
        
        # 检查是否有明显小于主体字号的单词（可能是下标/上标）
        for size in word_sizes:
            if size > 0 and size / max_size < threshold:
                return True
        
        return False
    
    def _detect_subscript_from_polygon(self, polygon: List[Tuple[float, float]], text: str) -> bool:
        """
        通过文本模式和polygon分析检测是否可能包含下标/上标
        
        由于 Azure OCR 不返回单词级别的数据，我们使用文本模式匹配来推断
        ...
        # 检测常见的下标模式
        subscript_patterns = [
            r'[A-Z][a-z]?(batch|max|min|avg|sum|total|mean|std|var|norm|dim)',  # Rbatch, Xmax等
            r'[a-zA-Z]\d+',  # X1, R2, a12等
            r'[a-zA-Z]{1,2}[a-z]{2,6}',  # Si, Ui, Ki等（单/双字母+小写词）
            r'\(\s*[sS][iI]\s*,',  # (Si, ...) 模式  
            r'[A-Z]{2}\d',  # SK1, XK2等
        ]
