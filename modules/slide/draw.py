...
from .data import BaseElement, ShapeElement, ConnectorElement, TextElement, TextParagraph, TextRun
from .transform import px_to_emu, px_to_pt, scale_font_size_for_pptx, split_polyline_to_segments
from .map import map_shape_type_to_pptx, map_arrow_type, map_arrow_type_with_size, map_dash_pattern
from .data import ConversionLogger, ConversionConfig, default_config
from .map import replace_font, PARALLELOGRAM_SKEW, DRAWIO_DEFAULT_FONT_FAMILY
from pathlib import Path
from latex2mathml.converter import convert as latex_to_mathml

# OMML namespace for Office Math
NS_OFFICE_MATH = 'http://schemas.openxmlformats.org/officeDocument/2006/math'
...
class PPTXWriter:
    ...
    def _add_shape(self, slide, shape: ShapeElement):
        ...
        # Handle image type
        if shape.shape_type == "image" and shape.image and shape.image.base64_data:
            import base64
            from io import BytesIO
            
            try:
                # Decode base64
                img_bytes = base64.b64decode(shape.image.base64_data)
                ...
                try:
                    if shape.id:
                        pic.name = f"drawio2pptx:image:{shape.id}"
                except Exception as e:
                    if self.logger:
                        self.logger.debug(f"Failed to set image name: {e}")
                
                return pic
            except Exception as e:
                if self.logger:
                    self.logger.warning(f"Failed to add image: {e}, falling back to rectangle")
                # Fall through to create a rectangle as fallback
        ...
        # Note: PowerPoint rotation is clockwise positive, draw.io rotation is counter-clockwise positive
        # So we need to negate the rotation value
        try:
            if shape.transform and shape.transform.rotation != 0.0:
                shp.rotation = shape.transform.rotation  
        except Exception as e:
            if self.logger:
    ...
    def _latex_to_omml(self, latex_content: str):
        """
        将 LaTeX 公式转换为 OMML (Office Math Markup Language)
        
        Args:
            latex_content: LaTeX 公式字符串 (不含定界符)
        
        Returns:
            OMML XML 元素，失败返回 None
        """
        try:
            # 1. LaTeX → MathML
            mathml_str = latex_to_mathml(latex_content)
            mathml_tree = ET.fromstring(mathml_str.encode('utf-8'))
            
            # 2. 加载 XSLT 转换器
            xslt_path = Path(__file__).parent / "MML2OMML.XSL"
            xslt_tree = ET.parse(str(xslt_path))
            ...
            if self.logger:
                self.logger.warning(f"Failed to convert LaTeX to OMML: {e}")
            return None
    
    def _insert_omml_to_paragraph(
        self, 
        p_element, 
        omml_element, 
        ...
        """
        将 OMML 公式插入到段落 XML 中，并应用样式
        
        Args:
            p_element: 段落的 XML 元素 (a:p)
            omml_element: OMML XML 元素
            font_color: 字体颜色
            font_size: 字体大小 (pt)
            font_family: 字体族
        """
        try:
            # 创建 a14:m 包装器
            a14_m = ET.SubElement(p_element, f'{{{NS_A14}}}m')
            
            # 设置公式样式（在添加 OMML 之前）
            if font_color or font_size or font_family:
                self._apply_omml_styles(omml_element, font_color, font_size, font_family)
            
            # 将 OMML 内容添加到 a14:m 中
            a14_m.append(omml_element)
            
        except Exception as e:
            if self.logger:
                self.logger.warning(f"Failed to insert OMML to paragraph: {e}")

    def _apply_omml_styles(
        self,
        omml_element,
        font_color: Optional[RGBColor] = None,
        font_size: Optional[float] = None,
        ...
        """
        应用样式到 OMML 公式的所有 run 元素
        在 PowerPoint 中，OMML 公式的样式通过 a:rPr 直接放在 m:r 中设置
        """
        try:
            # 遍历所有 m:r (run) 元素
            for r_elem in omml_element.iter(f'{{{NS_OFFICE_MATH}}}r'):
                # 如果有任何样式需要设置，先确保 a:rPr 存在
                if font_color or font_size or font_family:
                    # 查找或创建 a:rPr (DrawingML run properties) - 直接在 m:r 中
                    a_rPr = r_elem.find(_a('rPr'))
                    if a_rPr is None:
                        # 插入到 m:r 的开头
                        a_rPr = ET.Element(_a('rPr'))
                        r_elem.insert(0, a_rPr)
                    
                    # 设置颜色
                    if font_color:
                        # 移除已有的颜色设置
                        for old_fill in a_rPr.findall(_a('solidFill')):
                            a_rPr.remove(old_fill)
                        
                        # 添加 a:solidFill > a:srgbClr
                        solidFill = ET.SubElement(a_rPr, _a('solidFill'))
                        srgbClr = ET.SubElement(solidFill, _a('srgbClr'))
                        srgbClr.set('val', f'{font_color[0]:02X}{font_color[1]:02X}{font_color[2]:02X}')
                    
                    # 设置字体大小 - 使用 a:rPr 的 sz 属性（单位：百分之一磅 * 100）
                    if font_size:
                        a_rPr.set('sz', str(int(font_size * 100)))
                    
                    # 设置字体族 - 使用 a:latin 元素
                    if font_family:
                        # 移除已有的字体设置
                        for old_latin in a_rPr.findall(_a('latin')):
                            a_rPr.remove(old_latin)
                        
                        # 添加 a:latin 元素
                        latin = ET.SubElement(a_rPr, _a('latin'))
                        latin.set('typeface', font_family)
                    
        except Exception as e:
            if self.logger:
                self.logger.debug(f"Failed to apply OMML styles: {e}")
    ...
    def _set_text_frame(
        ...
        for para_data in paragraphs:
            ...
            for run_data in para_data.runs:
                # 检测是否为 LaTeX 公式
                if run_data.is_latex and run_data.latex_content:
                    omml = self._latex_to_omml(run_data.latex_content)
                    if omml is not None:
                        # 计算缩放后的字体大小
                        ...
                        if run_data.font_size:
                            ...
                            scaled_font_size = scale_font_size_for_pptx(12.0)
                        
                        # 插入 OMML 并应用样式
                        self._insert_omml_to_paragraph(
                            p._p, 
                            omml, 
                            font_color=run_data.font_color or RGBColor(0, 0, 0),
                            font_size=scaled_font_size,
                            font_family=run_data.font_family,
                        )
                        continue
                    else:
                        # 转换失败，回退为纯文本显示
                        run = p.add_run()
                        run.text = run_data.latex_content
                else:
                    run = p.add_run()
