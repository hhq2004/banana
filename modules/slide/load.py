...
from .data import (
    ...
    ConversionLogger, ConversionConfig, default_config
)
from .map import PARALLELOGRAM_SKEW, DRAWIO_DEFAULT_FONT_FAMILY
...
class StyleExtractor:
    ...
    def _parse_font_style(self, font_style_str: Optional[str]) -> dict[str, bool]:
        ...
        if font_style_str:
            try:
                ...
                for bit_pos, attr_name in self.FONT_STYLE_BITS.items():
    ...
    def extract_shape_type(self, cell: ET.Element) -> str:
        ...
        if shape_type:
            ...
            normalized = self.SHAPE_TYPE_MAP.get(shape_type)
            if normalized:
                return normalized
        ...
        if style:
            ...
            if first_part == "swimlane":
                return "swimlane"
            # Use dictionary mapping
            normalized = self.SHAPE_TYPE_MAP.get(first_part)
            if normalized:
                return normalized
            # Keep rhombus as-is
...
class DrawIOLoader:
    ...
    def extract_page_size(self, mgm_root: ET.Element) -> tuple:
        ...
        """
        page_width = mgm_root.attrib.get("pageWidth", 1920)
        page_height = mgm_root.attrib.get("pageHeight", 1080)
    ...
    def _extract_shape(self, cell: ET.Element, mgm_root: ET.Element) -> Optional[ShapeElement]:
        ...
        # Extract rotation attribute
        # draw.io: rotation=<degrees> (e.g., rotation=-90.0 for 90 degrees counter-clockwise)
        rotation = self.style_extractor.extract_style_float(style_str, "rotation", 0.0) or 0.0
        
        # Extract text
        text_paragraphs = self._extract_text(text_raw, font_color, style_str)
        
        # Extract image data if shape_type is "image"
        image_data = None
        if shape_type == "image":
            import re
            match = re.search(r'image=data:image/(\w+)[,;]([^";\s]+)', style_str)
            if match:
    ...
    def _extract_connector(self, cell: ET.Element, mgm_root: ET.Element, shapes_dict: Dict[str, ShapeElement]) -> tuple[Optional[ConnectorElement], List[TextElement]]:
        ...
        # 检查是否有 sourcePoint/targetPoint（自由箭头）
        geo = cell.find(".//mxGeometry")
        has_free_points = False
        source_point = None
        target_point = None
        ...
        # 如果没有 source/target 但有 sourcePoint/targetPoint，创建虚拟形状
        if (not source_id or not target_id) and source_point and target_point:
            has_free_points = True
            # 创建虚拟的点形状（0x0 大小）
            source_shape = ShapeElement(
                id=f"_virtual_source_{cell.attrib.get('id')}",
                x=source_point[0], y=source_point[1],
                w=0.0, h=0.0,
                shape_type="point"
            )
            target_shape = ShapeElement(
                id=f"_virtual_target_{cell.attrib.get('id')}",
                x=target_point[0], y=target_point[1],
                w=0.0, h=0.0,
                shape_type="point"
            )
        elif not source_id or not target_id:
            return None, []  # 既没有 source/target 也没有 sourcePoint/targetPoint
        else:
            source_shape = shapes_dict.get(source_id)
            target_shape = shapes_dict.get(target_id)
    ...
    def _extract_text(self, text_raw: str, font_color: Optional[RGBColor], style_str: str) -> List[TextParagraph]:
        ...
        if text_stripped.startswith('\\(') and text_stripped.endswith('\\)'):
            is_latex = True
            latex_content = text_stripped[2:-2].strip()
            print("is_latex")
            print(latex_content)
        ...
        if "<" in plain_text and ">" in plain_text:
            try:
                # 先把 <br> / <br/> 替换为 \n，保留换行信息
                import re
                br_replaced = re.sub(r'<br\s*/?>', '\n', plain_text, flags=re.IGNORECASE)
                wrapped = f"<div>{br_replaced}</div>"
                parsed = lxml_html.fromstring(wrapped)
        ...
        # Split by newlines (produced by <br> replacement above) into multiple paragraphs
        lines = plain_text.split('\n')
        # Remove empty trailing lines but keep internal empty lines for spacing
        ...
        for i, line in enumerate(lines):
            paragraph = TextParagraph(
                ...
                # Only set margins on the first paragraph (they apply to the whole text frame)
                spacing_top=spacing_top if i == 0 else None,
                spacing_left=spacing_left if i == 0 else None,
                spacing_bottom=spacing_bottom if i == 0 else None,
                spacing_right=spacing_right if i == 0 else None,
            )
