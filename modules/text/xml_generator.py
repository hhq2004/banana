...
class MxGraphXMLGenerator:
    ...
    def _build_style_string(self, cell_data: TextCellData) -> str:
        ...
        """
        is_vertical = self._is_vertical_text(cell_data)
        #print("!!!",cell_data.text,cell_data.is_vertical)
        if is_vertical:
            # styles = [
            #     "shape=rectangle",
            #     "whiteSpace=wrap",
            ...
            #     f"fontSize={int(cell_data.font_size)}",
            #     "fillColor=none",
            #     "strokeColor=none",
            #     "horizontal=0",  # 设置书写方向为垂直
            #     #"direction=east", # 配合 horizontal=0 实现从右向左的垂直书写？默认通常是 vertical-rl (CJK standard)
            #     # direction=east 在 horizontal=0 下通常对应 vertical-lr 还是 vertical-rl?
            #     # draw.io 中 "Vertical" 默认是从右到左换行 (CJK常规)。
            # ]
            styles = [
        ...
        # 旋转角度
        if is_vertical:
            if not self._contains_chinese(cell_data.text):
                styles.append("rotation=-90")
        elif cell_data.rotation != 0:
            styles.append(f"rotation={cell_data.rotation}")
        
        return ";".join(styles) + ";"
    
    def _is_vertical_text(self, cell_data: TextCellData) -> bool:
        """判断是否为竖排文字（优先使用 is_vertical，兜底使用旋转角度）"""
        if cell_data.is_vertical is not None:
            return bool(cell_data.is_vertical)
        return abs(abs(cell_data.rotation) - 90) < 15
    ...
    def _add_text_cell(self, root: ET.Element, cell_data: TextCellData) -> None:
        ...
        # 处理垂直中文文本：一个字加一个换行
        is_vertical = self._is_vertical_text(cell_data)
        has_chinese = self._contains_chinese(cell_data.text)
        
        if is_vertical and has_chinese and not cell_data.is_latex:
            # 垂直中文文本，逐字换行
            escaped_chars = [self._escape_text(c, False) for c in cell_data.text]
            cell.set("value", "<br>".join(escaped_chars))
        else:
