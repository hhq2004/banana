...
class FormulaProcessor:
    ...
    def is_valid_formula(self, latex_text: str) -> bool:
        ...
        # array/matrix 结构：检查内部是否包含有意义的数学内容
        if '\\begin{array}' in text or '\\begin{matrix}' in text:
            # 提取 array/matrix 内部的内容
            inner_match = re.search(r'\\begin\{(array|matrix)\}[^}]*\}?(.*?)\\end\{\1\}', text, flags=re.DOTALL)
            if inner_match:
                inner_content = inner_match.group(2)
                # 检查内部是否包含数学符号（下标、等号、希腊字母等）
                has_math_inside = any(sym in inner_content for sym in ['_{', '^{', '=', '\\mathbf', '\\mathrm', '\\alpha', '\\beta', '\\tau', '\\kappa', 'f_', 'M_', 'v_', 'c_'])
                if not has_math_inside:
                    # 检查是否只是简单的虚线或分隔符
                    cleaned = re.sub(r'[\\{}\[\]\s\|&]', '', inner_content)
                    if len(cleaned) < 5:
                        return False
        ...
        # 纯粗体短文本（不含下标/上标）
        # 但如果包含下标/上标，则是有效的数学变量，如 \mathbf{x}_{0}
        if text.startswith('\\mathbf{') and len(text) < 15:
            # 检查是否包含下标或上标
            if '_{' not in text and '^{' not in text and '_' not in text[8:] and '^' not in text[8:]:
                return False
