...
class XMLMerger(BaseProcessor):
    ...
    def process(self, context: ProcessingContext) -> ProcessingResult:
        ...
        if hasattr(context, 'intermediate_results') and context.intermediate_results:
            scale_factor = context.intermediate_results.get('upscale_factor', 1.0)
        
        # 计算原始画布尺寸（如果有超分，需要缩小回原始尺寸）
        canvas_width = context.canvas_width
        canvas_height = context.canvas_height
        if scale_factor > 1.0:
            canvas_width = int(context.canvas_width / scale_factor)
            canvas_height = int(context.canvas_height / scale_factor)
            self._log(f"画布尺寸调整: {context.canvas_width}x{context.canvas_height} → {canvas_width}x{canvas_height} (缩放因子: {scale_factor:.2f})")
        
        # 收集所有XML片段
        fragments = self._collect_fragments(context, scale_factor)
        ...
        stem_source = None
        if hasattr(context, "intermediate_results") and context.intermediate_results:
            stem_source = context.intermediate_results.get("original_image_path")
        if not stem_source:
            stem_source = getattr(context, "original_image_path", None)
        if not stem_source:
            stem_source = context.image_path

        stem = Path(stem_source).stem if stem_source else "merged"
    ...
    def _collect_fragments(self, context: ProcessingContext, scale_factor: float = 1.0) -> List[XMLFragment]:
        """
        收集所有XML片段
        
        收集顺序：
        1. 从 context.xml_fragments 获取（图形元素，需要缩小坐标）
        2. 从 context.elements 中提取有 xml_fragment 的元素（图形元素，需要缩小坐标）
        3. 从 context.intermediate_results['text_xml'] 提取文字 XML（文字元素，保持原坐标）
        
        Args:
            context: 处理上下文
            scale_factor: 超分缩放因子，图形元素坐标需要除以此值
        """
        fragments = []
        
        # 方式1: 直接从 xml_fragments 获取（图形元素）
        if hasattr(context, 'xml_fragments') and context.xml_fragments:
            for frag in context.xml_fragments:
                if scale_factor > 1.0:
                    # 缩小图形元素坐标
                    scaled_xml = self._scale_xml_coordinates(frag.xml_content, 1.0 / scale_factor)
                    scaled_bbox = self._scale_bbox(frag.bbox, 1.0 / scale_factor) if frag.bbox else None
                    fragments.append(XMLFragment(
                        element_id=frag.element_id,
                        xml_content=scaled_xml,
                        layer_level=frag.layer_level,
                        bbox=scaled_bbox,
                        element_type=frag.element_type
                    ))
                else:
                    fragments.append(frag)
        
        # 方式2: 从 elements 中提取（图形元素）
        if context.elements:
            for elem in context.elements:
                if elem.has_xml():
                    ...
                    if elem.id not in existing_ids:
                        ...
                        if scale_factor > 1.0:
                            # 缩小图形元素坐标
                            xml_content = self._scale_xml_coordinates(xml_content, 1.0 / scale_factor)
                            bbox = self._scale_bbox(bbox, 1.0 / scale_factor) if bbox else None
        ...
        if scale_factor > 1.0:
            self._log(f"图形元素坐标已按 1/{scale_factor:.2f} 缩小")
        
        # 方式3: 从 text_xml 提取文字片段（文字元素，保持原坐标不变）
        if hasattr(context, 'intermediate_results') and context.intermediate_results:
            ...
            if text_xml:
                ...
                self._log(f"从文字XML中提取了 {len(text_fragments)} 个文字片段 (坐标保持原图尺寸)")
        ...
        try:
            ...
            if geometry is not None:
                ...
                for attr in ['x', 'y', 'width', 'height']:
                    val = geometry.get(attr)
                    if val:
                        try:
                            scaled_val = float(val) * scale
                            geometry.set(attr, str(int(scaled_val)))
                        except ValueError:
                            pass
                ...
                for point in geometry.findall('.//mxPoint'):
                    for attr in ['x', 'y']:
                        ...
                        if val:
                            try:
                                scaled_val = float(val) * scale
                                point.set(attr, str(int(scaled_val)))
                            except ValueError:
                                pass
    ...
    def _extract_text_fragments_from_xml(self, text_xml: str) -> List[XMLFragment]:
        ...
        文字坐标保持不变（基于原图尺寸），图形元素会在 _collect_fragments 中缩小
        """
        fragments = []
